...
ElementPeerList::ElementPeerList(core::Download* d) :
  ...
  connection_list->signal_connected().push_back([this](auto p) { receive_peer_connected(p); });
  connection_list->signal_disconnected().push_back([this](auto p) { receive_peer_disconnected(p); });
...
ElementPeerList::~ElementPeerList() {
  ...
  connection_list->signal_connected().pop_back();
  connection_list->signal_disconnected().pop_back();
